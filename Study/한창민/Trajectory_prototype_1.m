...
for i = 1:size(obs_global,1)
    plot(obs_global(i,1), obs_global(i,2), 's', ...
         'MarkerSize', 10, ...
         'MarkerEdgeColor', 'k', ...
         'MarkerFaceColor', 'r');  % 빨간색 장애물
end



% 반복 시각화용 변수 초기화
dot_plot = []; best_traj_plot =[]; traj_plots=[];
...
function fplist = calc_frenet_paths(si, si_d, si_dd, sf_d, sf_dd, di, di_d, di_dd, df_d, df_dd, opt_d)
    ...
    TARGET_SPEED=2;
    if isempty(DF_SET), DF_SET = linspace(-6, 6, 3); end
    if isempty(DT), DT = 1; end
    if isempty(DT_T), DT_T = 0.5; end
    if isempty(MIN_T), MIN_T = 1.0; end
    if isempty(MAX_T), MAX_T = 2.0; end
