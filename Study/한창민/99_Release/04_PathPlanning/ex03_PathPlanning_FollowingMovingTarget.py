...
class LeadingVehiclePos(object):
    ...
    def update(self, pos_lead, vx, yawrate, dt):
        self.PosArray.append(pos_lead[0])  # [[x, y]] → [x, y]
        if len(self.PosArray) > self.max_num_array:
            self.PosArray.pop(0)

def HeadingAngleEstimation(coeff_path, PosArray):
    if len(PosArray) == 0:
        return 0.0  # 초기값 fallback

    # 현재 가장 최근 위치 기준으로 추정
    x0 = PosArray[-1][0]

    # 도함수 계수 계산 (coeff_path: [a0, a1, a2, a3])
    deriv_coeff = [i * coeff_path[i] for i in range(1, len(coeff_path))]

    # 도함수 평가
    dy_dx = sum(c * x0 ** i for i, c in enumerate(deriv_coeff))

    # 헤딩 각도 (rad)
    heading_angle = np.arctan(dy_dx)
...
def TargetFollowingPath(PosArray):
    if len(PosArray) < 3:
        return np.array([0.0, 0.0, 0.0, 0.0])  # 초기 fallback 경로

    pos_array_np = np.array(PosArray)
    x = pos_array_np[:, 0]
    y = pos_array_np[:, 1]

    coeff = np.polyfit(x, y, 3)
    return coeff[::-1]  # 상수항부터 정렬
