...
class LaneWidthEstimator(object):
    def __init__(self, Lw_init=3.0):
        self.Lw = Lw_init  # 초기 차선 폭

    def update(self, coeff_L, coeff_R, isLaneValid_L, isLaneValid_R):
        if np.sum(isLaneValid_L) >= 5 and np.sum(isLaneValid_R) >= 5:
            mid_x = np.linspace(0.0, 5.0, num=10)
            y_L = np.polyval(coeff_L[::-1], mid_x)
            y_R = np.polyval(coeff_R[::-1], mid_x)
...
def EitherLane2Path(coeff_L, coeff_R, isLaneValid_L, isLaneValid_R, Lw):
    ...
    if valid_L and valid_R:
        ...
        y_vals_L = np.polyval(coeff_L[::-1], x_vals)
        y_path = y_vals_L - max(Lw, MIN_LANE_WIDTH) / 2.0
        coeff_path = np.polyfit(x_vals, y_path, 3)[::-1]
        return coeff_path

    elif valid_R:
        y_vals_R = np.polyval(coeff_R[::-1], x_vals)
        y_path = y_vals_R + max(Lw, MIN_LANE_WIDTH) / 2.0
        coeff_path = np.polyfit(x_vals, y_path, 3)[::-1]
        return coeff_path
