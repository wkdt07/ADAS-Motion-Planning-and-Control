...
def Global2Local(global_points, yaw_ego, X_ego, Y_ego):
    # Code
    rotation_matrix = np.array([[np.cos(-yaw_ego), -np.sin(-yaw_ego)], [np.sin(-yaw_ego), np.cos(-yaw_ego)]])
    local_points = []
    for point in global_points:
        local_point = np.dot(rotation_matrix, np.array([point[0] - X_ego, point[1] - Y_ego]))
