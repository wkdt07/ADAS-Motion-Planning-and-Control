...
class PID_Controller_ConstantSpace(object):
    def __init__(self, step_time, target_x, ego_x, constantSpace=0.0, P_Gain=0.12, D_Gain=0.225, I_Gain=0.008):
        # Code
        self.dt = step_time
        if constantSpace == 0.0:
    ...
    def ControllerInput(self, target_x, ego_x):
        # Code
        current_space = target_x - ego_x
        self.error = current_space - self.space

        self.i_err += self.error * self.dt
        self.error_Gap = (self.error - self.prev_error) / self.dt
