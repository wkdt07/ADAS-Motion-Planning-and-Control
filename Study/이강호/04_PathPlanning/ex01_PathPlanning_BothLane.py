...
def Global2Local(global_points, yaw_ego, X_ego, Y_ego):
    ...
    for i in range(len(Points)):
        ...
        LocalPoints.append([x_local, y_local])
    return LocalPoints
        
# Polynomial fitting (n_th order)
def Polyfit(points, num_order):
    """
    Fit a polynomial of a given order to the provided points.

    Args:
        points (list): List of local waypoints as [x, y].
        num_order (int): The degree of the polynomial (e.g., 3 for a cubic polynomial).

    Returns:
        list: Coefficients of the fitted polynomial.
    """
    x = [p[0] for p in points]
    y = [p[1] for p in points]
    
    # Fit a polynomial of the given order to the points
...
def BothLane2Path(coeff_L, coeff_R):
    # 차선의 중간을 그리기
    coeff_path = [(l + r) / 2 for l, r in zip(coeff_L, coeff_R)]
    return coeff_path
...
class PurePursuit(object):
    def __init__(self, L=4.0, lookahead_time=3.0):
        self.L = L
        self.epsilon = 0.001
