...
class RRTStar(object):
    ...
    def get_nearest(self, rand_node):
        ...
        for node_id in self.G.nodes:
            ...
            dist = np.hypot(rand_node[0] - x, rand_node[1] - y)
            if dist < min_dist:
                min_dist = dist
    ...
    def steer(self, node_from, node_to, u=None):
        ...
        eta = self.config["eta"]
        if dist <= eta:
            return node_to[0], node_to[1]
        else:
            theta = np.arctan2(dy, dx)
            return node_from[0] + eta * np.cos(theta), node_from[1] + eta * np.sin(theta)
    ...
    def is_collision_free(self, node_from, node_to, obstacles, step=0.2):
        ...
        for i in range(steps + 1):
            x = node_from[0] + dx * i / steps
            y = node_from[1] + dy * i / steps
            for obs in obstacles:
                if obs.is_inside(x, y):  # 각 obs는 .is_inside(x, y) 메서드를 가지고 있어야 함
                    return False
        return True
    ...
    def get_near_node_ids(self, new_node, draw):
        ...
        if n == 0:
            return []
        gamma = self.config["gamma_rrt_star"]
        r = gamma * (np.log(n) / n) ** (1.0 / 2.0)  # 2D 공간 가정
        near_ids = []
        for node_id in self.G.nodes:
            x = self.G.nodes[node_id]['x']
            y = self.G.nodes[node_id]['y']
            dist = np.hypot(new_node[0] - x, new_node[1] - y)
            if dist <= r:
    ...
    def get_distance(self, node_from_id, node_to_id):
        x1, y1 = self.G.nodes[node_from_id]['x'], self.G.nodes[node_from_id]['y']
        x2, y2 = self.G.nodes[node_to_id]['x'], self.G.nodes[node_to_id]['y']
