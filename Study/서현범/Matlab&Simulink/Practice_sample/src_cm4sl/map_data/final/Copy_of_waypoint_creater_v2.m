...
for route_idx = 1:numel(output)
    ...
    for point_idx = 1:size(route_data, 1)
        ...
        if size(current_profile, 1) >= min_num_waypoints
            ...
            % shb
            road_profiles{end+1} = struct( ...
                                            'id', numel(road_profiles) + 1, ...
                                            'waypoint', current_profile, ...
                                            'numWaypoint', size(current_profile, 1), ...
                                            'route', route_idx);
    ...
    % end
        if ~isempty(current_profile)
            road_profiles{end} = struct( ...
                'id', road_profiles{end}.id, ...
                'waypoint', [road_profiles{end}.waypoint; current_profile], ...
                'numWaypoint', size(road_profiles{end}.waypoint, 1) + size(current_profile, 1), ...
                'route', route_idx);  % ✅ 누락 방지
        end    
...
figure;
hold on;
colors = lines(numel(output));  % 각 route마다 다른 색상
...
for profile_idx = 1:numel(road_profiles)
    ...
    plot(waypoints(:, 1), waypoints(:, 2), '.-', 'Color', colors(route_id,:));  % route별 색상
    text(waypoints(1,1), waypoints(1,2), ['R', num2str(route_id)], 'Color', colors(route_id,:), 'FontSize', 10)
...
by = [ymax, ymax, ymin, ymin, ymax];  % 반시계방향 닫힘

% 경계 그리기
plot(bx, by, 'k-', 'LineWidth', 2)
text(xmin, ymax, 'P1', 'FontSize', 10, 'Color', 'r')
text(xmax, ymax, 'P2', 'FontSize', 10, 'Color', 'r')
text(xmax, ymin, 'P3', 'FontSize', 10, 'Color', 'r')
text(xmin, ymin, 'P4', 'FontSize', 10, 'Color', 'r')

%%
% Set plot title and labels
title('Extracted Profiles and Waypoints');
xlabel('X-coordinate');
ylabel('Y-coordinate');
grid on;
hold off;